...
function handleChange(evt) {
  ...
  for (var i = 0; i < sel.options.length; i++) {
    var results = sel.options[i].text;
      ...
      if (evt.target.readyState == FileReader.DONE) {
        ...
        if (lines[0] == "") {
          for (index = 1; index < lines.length; index++) {
            ...
            var text = "Route ";
            if(json.locations){
              for(var locIndex = 0; locIndex < json.locations.length; locIndex++){
                var locVal = json.locations[locIndex];
                text += " ("+ locVal.lon + "," + locVal.lat + ") ";
                text += locVal.city || " ";
                text += locVal.name || " ";
              }
            }
...
app.controller('RouteController', function($scope, $rootScope, $sce, $http) {
 ...
 // allow hash links
  ...
  var update = function(show, locs, costing) {
    ...
    document.getElementById('permalink').innerHTML = "<a href='http://michacomap.westeurope.cloudapp.azure.com/demos/routing/index.html" + window.location.hash + "' target='_top'>Route Permalink</a>";
  };
